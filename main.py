...
# 2. ESTILO CSS PARA MULTIDENUNCIA
st.markdown("""
    <style>
...
def limpiar_texto(t):
    if not t: return ""
    return ''.join(c for c in unicodedata.normalize('NFD', str(t).strip()) if unicodedata.category(c) != 'Mn').lower()
...
mapa_emojis = {"vmp": "🛴", "alcohol": "🍺", "movil": "📱", "drogas": "💊"}
...
try:
    ...
    if not st.session_state['autenticado']:
        with st.form(key='login_form'):
            ...
            if st.form_submit_button(label='ENTRAR'):
                user = usuarios_df[(usuarios_df['nombre'].astype(str).str.lower() == nombre_input.lower().strip()) & (usuarios_df['contraseña'].astype(str) == contrasena_input.strip())]
                if not user.empty:
    ...
    # --- 6. APP PRINCIPAL ---
        ...
        if busqueda:
            ...
            resultado = protocolos_df[protocolos_df['texto_busqueda'].apply(lambda texto: all(t in texto for t in terminos))]
            ...
            if not resultado.empty:
                for _, row in resultado.iterrows():
                    ...
                    with st.expander(f"{emoji} {titulo} | {norma_p} Art. {art_p}"):
                        ...
                        with col1:
                            ...
                            codigos = [f"{n_list[i].strip()} {a_list[i].strip() if i<len(a_list) else ''} {ap_list[i].strip() if i<len(ap_list) else ''} {o_list[i].strip() if i<len(o_list) else ''}".strip() for i in range(len(n_list))]
                            st.markdown(f"<div class='dato-importante'>{'<br>'.join(codigos)}</div>", unsafe_allow_html=True)
                        ...
                        with col4:
                            ...
                            multas = [f"{m_raw[i].strip()}€ ({rd_raw[i].strip() if i<len(rd_raw) else '?'}€)" for i in range(len(m_raw))]
                            st.markdown(f"<div class='dato-importante'>{'<br>'.join(multas)}</div>", unsafe_allow_html=True)
